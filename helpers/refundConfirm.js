...
module.exports = function(callback) {
  ...
  cs.Purchase({}, {fromBlock: 0, toBlock: "latest"}).get(async (err, res) => {
    ...
    for (var idx in res.length){
      confirmedPayoutAddresses[idx.]
    }
    ...
    if (totalRaised.greaterThan(minToRaise) || currentBlock.lessThan(saleEndBlock)){
    ...
    for (idx = 0; idx < transactionCount; idx++){
      ...
      // Check the tx goes to an expected address
